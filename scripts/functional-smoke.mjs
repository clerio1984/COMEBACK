...
const tests = [
  ...
  ['Server: push endpoint requires authentication, not admin role', /app\.post\(["']\/api\/trigger-push["'][\s\S]*?requireAuthenticatedUser/.test(files.server) && !/app\.post\(["']\/api\/trigger-push["'][\s\S]*?requireAdmin/.test(files.server)],
  ['Server: SMS endpoint requires authentication, not admin role', /app\.post\(["']\/api\/trigger-sms["'][\s\S]*?requireAuthenticatedUser/.test(files.server) && !/app\.post\(["']\/api\/trigger-sms["'][\s\S]*?requireAdmin/.test(files.server)],
